...
{
    public class IndiceSeguimientoRepository : IIndiceSeguimientoRepository
    {
        private readonly IProyectoContext _controlHorarioContext;
        public IndiceSeguimientoRepository(IProyectoContext controlHorarioContext)
        {
        ...
        {
            var result = 0;
            result = await _controlHorarioContext.IndiceSeguimiento.CountAsync(x => x.IseId == id);
            return result > 0;
        } 
        ...
        {
            var result = 0;
            //result = await _controlHorarioContext.IndiceSeguimiento.CountAsync(x => x.CarNombre.ToUpper() == nombre.ToUpper());
            return result > 0;
        }
        ...
        {
            return await _controlHorarioContext.IndiceSeguimiento
                .Where(x => x.IseId == id)
                .Select(x => new IndiceSeguimientoModel
                {
                    IseId = x.IseId,
                    EspId = x.EspId,
                    IseNumero = x.IseNumero,
                    UsuId = x.UsuId
                }).FirstOrDefaultAsync();
        }
        ...
        {
            var result = new ListadoPaginadoModel<IndiceSeguimientoModel>();
            var query = _controlHorarioContext.IndiceSeguimiento.Select(x => new IndiceSeguimientoModel()
            {
                IseId = x.IseId,
                EspId = x.EspId,
                IseNumero = x.IseNumero,
        ...
        public async Task<List<ListModel>> GetList()
        {
            ...
            public async Task<long> Add(IndiceSeguimiento entity)
        {
            await _controlHorarioContext.IndiceSeguimiento.AddAsync(entity);
            await _controlHorarioContext.SaveChangesAsync();
            return entity.IseId;
        }

        //actaliza un dato 
        public async Task<long> Update(IndiceSeguimiento entity)
        {
            _controlHorarioContext.IndiceSeguimiento.Update(entity);
            await _controlHorarioContext.SaveChangesAsync();
            return entity.IseId;
        }
        ...
        public async Task<long> Delete(List<long> ids)
        {
            ...
            {
                IndiceSeguimiento IndiceSeguimiento = null;
                try
                {
                    foreach (var item in ids)
                    {
                        IndiceSeguimiento = await _controlHorarioContext.IndiceSeguimiento.Where(x => x.IseId == item).FirstAsync();
                        _controlHorarioContext.IndiceSeguimiento.Remove(IndiceSeguimiento);
                    }
                    await _controlHorarioContext.SaveChangesAsync();
