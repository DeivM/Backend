...
namespace WebApi.Controllers
{
    ...
    public class EspecialidadeController : BaseApiController
    {
        ...
        [ResponseType(typeof(RespuestaModel<ListadoPaginadoModel<EspecialidadeModel>>))]
        [HttpGet]
        public async Task<IActionResult> GetAll( int quantity = 10, int page = 0, string orderby = "EspId", string ascdesc = "descending", string search = null)
        {
            var resultado = new RespuestaModel<ListadoPaginadoModel<EspecialidadeModel>>
            {
                Data = await _EspecialidadeService.GetAll(quantity, page, orderby, ascdesc, search)
            };
            return Ok(resultado);
        }
        ...
        [ResponseType(typeof(RespuestaModel<EspecialidadeData>))]
        [HttpGet]
        [Route("GetData")]

        public async Task<IActionResult> GetData(int id = 0)
        {
            var resultado = new RespuestaModel<EspecialidadeData>
            {
                Data = await _EspecialidadeService.GetData(id)
            };
        ...
        [HttpPost]
        public async Task<IActionResult> Post(EspecialidadeRequest data)
        {
            var resultado = new RespuestaModel<long?>
            {
                Data = await _EspecialidadeService.Add(data)
            };
            return Ok(resultado);
        }
        ...
        [HttpPut]
        public async Task<IActionResult> Update(int id, EspecialidadeRequest data)
        {
            data.EspId = id;
            var resultado = new RespuestaModel<long?>
            {
                Data = await _EspecialidadeService.Update(data)
            };
            return Ok(resultado);
        }
        ...
        public async Task<IActionResult> Delete(List<long> ids)
        {
            ...
            await _EspecialidadeService.Delete(ids);
            return Ok(resultado);    
        }
