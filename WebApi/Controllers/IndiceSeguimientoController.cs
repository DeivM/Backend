...
{
    [Authorize]
    public class IndiceSeguimientoController : BaseApiController
    {
        private readonly IIndiceSeguimientoService _IndiceSeguimientoService;
        public IndiceSeguimientoController(IIndiceSeguimientoService IndiceSeguimientoService)
        {
            _IndiceSeguimientoService = IndiceSeguimientoService;
        }
        ...
        [ResponseType(typeof(RespuestaModel<ListadoPaginadoModel<IndiceSeguimientoModel>>))]
        [HttpGet]
        public async Task<IActionResult> GetAll( int quantity = 10, int page = 0, string orderby = "IseId", string ascdesc = "descending", string search = null)
        {
            var resultado = new RespuestaModel<ListadoPaginadoModel<IndiceSeguimientoModel>>
            {
                Data = await _IndiceSeguimientoService.GetAll(quantity, page, orderby, ascdesc, search)
            };
            return Ok(resultado);
        }
        ...
        [ResponseType(typeof(RespuestaModel<IndiceSeguimientoData>))]
        [HttpGet]
        [Route("GetData")]

        public async Task<IActionResult> GetData(int id = 0)
        {
            var resultado = new RespuestaModel<IndiceSeguimientoData>
            {
                Data = await _IndiceSeguimientoService.GetData(id)
            };
        ...
        [HttpPost]
        public async Task<IActionResult> Post(IndiceSeguimientoRequest data)
        {
            var resultado = new RespuestaModel<long?>
            {
                Data = await _IndiceSeguimientoService.Add(data)
            };
            return Ok(resultado);
        }
        ...
        [HttpPut]
        public async Task<IActionResult> Update(int id, IndiceSeguimientoRequest data)
        {
            data.IseId = id;
            var resultado = new RespuestaModel<long?>
            {
                Data = await _IndiceSeguimientoService.Update(data)
            };
            return Ok(resultado);
        }
        ...
        public async Task<IActionResult> Delete(List<long> ids)
        {
            ...
            await _IndiceSeguimientoService.Delete(ids);
            return Ok(resultado);    
        }
